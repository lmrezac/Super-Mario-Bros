...
public class Utilities{
	...
	public static BufferedImage lighter(BufferedImage in){
		BufferedImage bimg = new BufferedImage(in.getWidth(), in.getHeight(), in.getType());
		for(int y = 0; y<in.getHeight(); y++){
			for(int x = 0; x<in.getWidth(); x++){
				int rgb = in.getRGB(x, y);
				// bimg.setRGB(x, y, (rgb-amt)/2);
	...
	private static Color contrast(Color c,int factor){
		int r = c.getRed();
		int g = c.getGreen();
		int b = c.getBlue();
	...
	public static ImageIcon addImages(ImageIcon base, ImageIcon add, String direction){
		...
		if(direction.equals("down")){
			int width;
			if(base.getIconWidth()>add.getIconWidth())
				width = base.getIconWidth();
			else
				width = add.getIconWidth();
			...
			int height;
			if(base.getIconHeight()>add.getIconHeight())
				height = base.getIconHeight();
			else
				height = add.getIconHeight();
