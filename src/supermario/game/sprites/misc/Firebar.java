...
public class Firebar extends Sprite implements EnemyHolder{
	...
	private void setMetalType(final Tile tile){
		...
		/*
		 * if (levelType == 2) { this.metal = this.game.textures.stoneMetal; }
		 * else if (levelType == 0 || levelType == 4 || levelType == 5 ||
		 * levelType == 6) {
		 */ this.metal = game.textures.getLevelTypeAlt(levelType, this.game.textures.lightMetal);
		/*
		 * } else if (levelType == 1) { this.metal =
		 * this.game.textures.darkMetal; }
		 */
		/* else */ if(levelType==3){
			this.metal = null;
			tile.solid = false;
